...
pretrained = 'ckpts/mae_pretrain_vit_base.pth'
model = dict(
    type='MapTRv2',
    use_grid_mask=True,
    ...
    img_backbone=dict(
        ...
        img_neck = dict(
            type='FPN',
            in_channels=[384, 384, 384, 384],  # Все уровни имеют 384 канала
            out_channels=256,                 # Количество каналов на выходе, например, 256
            start_level=0,                      # Начинаем с самого первого уровня
            add_extra_convs='on_output',        # Добавляем дополнительные свертки на выходе
            num_outs=4,             # Количество выходных уровней
    ...
    pts_bbox_head=dict(
        ...
        loss_seg=dict(type='DiceLoss', 
            # pos_weight=4.0,
            loss_weight=1.0),
        loss_pv_seg=dict(type='DiceLoss', 
                    # pos_weight=1.0,
                    loss_weight=2.0),
